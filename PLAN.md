...
### 2026-04-21
- Completed:
  - 对 desktop Hero 蓝色块宽度做小步回调（`61.2% -> 60%`），降低 1280 顶区过宽占比。
  - 复跑 check/verify 与固定审图流程，确认 430 保持 17.09%，1280-top 指标出现微弱改善（22.87% -> 22.86%）。
- Next:
  - 继续围绕 1024/1280 做更细粒度联动（主标记 left 与蓝块宽度同步 0.5% 级别扫描）。
  - 保持移动端参数冻结，避免再次引入 430 回退。
- Blocked:
  - 无真实阻塞；当前进入低收益微调阶段。
- Verification:
  - `npm run check`
...
- Completed:
  - 回滚上一轮 `<=640` hero 主标记/蓝块过度压缩参数，恢复 `66%` 与 `60/56` 组合，消除 430 顶区回退。
  - 复跑 check/verify 并确认 430 指标从 17.40% 回到 17.09% 基线。
- Next:
  - 在不动 430 基线的前提下，转向 1024/1280 的 hero 局部位移与蓝块宽度联动实验。
  - 后续所有移动端试验以 A/B 临时分支验证，避免直接污染主校准曲线。
- Blocked:
  - 无真实阻塞；问题集中在 desktop 断点收敛慢。
- Verification:
  - `npm run check`
  - `npm run verify:ark`
